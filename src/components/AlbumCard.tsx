        ...
        <div className="mt-10 flex gap-6 text-lg ">
          ...
          <div className="">
            ...
            <div
              ...
              className="relative box-border w-full cursor-pointer border-2 border-gray-600 bg-black p-2 "
            >
              {returnTracklist()}
              <div
                className={`${
                  showTracklist ? "block" : "hidden"
                } absolute left-0 top-0 h-44 w-[100%] overflow-scroll overflow-x-hidden  `}
              >
                {currentAlbum?.tracks?.track.map(
                  (curTrack: any, index: number) => {
                    return (
                      <div
                        key={index}
                        className="flex w-full justify-between border-2 border-gray-600 bg-black p-2 "
                      >
                        <div className="flex gap-2">
                          <p>{index + 1}.</p>
                          <p>{curTrack.name}</p>
